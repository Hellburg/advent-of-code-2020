...
pid:921038878";
    ...
    let required: [&str; 7] = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];

    let passports_string: Vec<&str> =
        input
            .split("\n\n")
            .filter(|x| !x.is_empty())
            .collect();
    ...
    for pass in passports_string {
        ...
        let pass_split_on_newline: Vec<&str> = pass.split("\n").collect();
        for split_on_newline in pass_split_on_newline {
            ...
            for split_on_mellanslag in split_again {
                let part: Vec<&str> = split_on_mellanslag.split(":").collect();
                passport.insert(*part.get(0).unwrap(), *part.get(1).unwrap());
            }
        }
